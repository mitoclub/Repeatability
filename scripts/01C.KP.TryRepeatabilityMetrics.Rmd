...
## 3. naive, greedy and redundant main function, which extracts all motifs, overlapping the position within the defined interval, and get all repeats for each motif; overlapped repeats are redundant (long ones may contain short ones, motif can be self linked as a repeat == identical sequences with identical coordinates with 0 humming distance) - we keep all these redundancy at this step. We filter results as next step and have a freedom to make different steps. 
...
# Main routine for fixed nucleotide only
...
## 4. potential filtratio steps (To DO): (i) remove rows, where motif is similar to itself; (ii) remove nested short repeats within the long ones
